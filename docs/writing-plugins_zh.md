...
- `handlers.py` — 实现在 IDA Worker 进程中运行的 handler 函数
...
| `kind:unsafe` | 破坏性或不可逆操作 | 自动创建 undo point；在 MCP schema 中标记 |

写入工具自动获得 undo 支持——如果 AI 犯了错误，`undo` 可以撤销修改。无需为单次修改手动保存或快照。
